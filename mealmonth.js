...
fetch("https://open.neis.go.kr/hub/mealServiceDietInfo?KEY=dba1fc9266654cf5921022efad652d33&Type=json&pIndex=1&pSize=100&ATPT_OFCDC_SC_CODE=B10&SD_SCHUL_CODE=7041169&MLSV_FROM_YMD=" + start + "&MLSV_TO_YMD="+ end)
...
.then((data) => {
    ...
    var i = 0
    while(typeof data.mealServiceDietInfo[1].row[i] !== 'undefined'){
    
    var str = JSON.stringify(data.mealServiceDietInfo[1].row[i].DDISH_NM);
    ...
    re = re.replace(/[0-9]/g, '');
    re = re.replace(/\./g, '');
    var MLSV_YMD = JSON.stringify(data.mealServiceDietInfo[1].row[i].MLSV_YMD);
    var AA_d = Number(MLSV_YMD.substring(7, 9)) + day;
